...
for angle in angles:
    for maxForce in maxForces:
        df = pd.read_excel('{}deg_{}N_1.xls'.format(angle, maxForce))
        ...
        for target in [df['ir'], df['baro']]:
            dx = np.mean(np.diff(df['time_sensor'].values))
            # print (dx)
            shift = (np.argmax(signal.correlate(target.values, df['newton'].values[1:1850],mode='same')) - 1850)*dx
            shifts.append(shift)
