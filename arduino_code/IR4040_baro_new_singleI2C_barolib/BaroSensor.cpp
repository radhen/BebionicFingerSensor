...
{ 
  ...
  for(int i = 0; i < 7; i++) {
    ...
    if(err)
      return;
    int req = Wire.requestFrom(address, 2);
    if(req != 2) {
      err = ERR_BAD_READLEN;
      return;
    }
...
bool BaroSensorClass::getTempAndPressure(float *temperature, float *pressure, TempUnit tempScale, BaroOversampleLevel level, byte address)
{
  ...
  int32_t d2 = takeReading(CMD_START_D2(level), level, byte(address));
...
uint32_t BaroSensorClass::takeReading(uint8_t trigger_cmd, BaroOversampleLevel oversample_level, byte address)
{
  Wire.beginTransmission(byte(address));
  ...
  Wire.beginTransmission(byte(address));
  Wire.write(byte(CMD_READ_ADC));
  err = _endTransmission(false);
  if(err)
    return 0;
  int req = Wire.requestFrom(byte(address), 3);
  if(req != 3)
    req = Wire.requestFrom(byte(address), 3); // Sometimes first read fails...?
  if(req != 3) {
    err = ERR_BAD_READLEN;
    return 0;
  }
...
//  Serial.println(takeReading(CMD_START_D1(OSR_8192), OSR_8192, byte(address)));
//  Serial.print(F("d2 first = 0x"));
//  Serial.println(takeReading(CMD_START_D2(OSR_8192), OSR_8192, byte(address)));
