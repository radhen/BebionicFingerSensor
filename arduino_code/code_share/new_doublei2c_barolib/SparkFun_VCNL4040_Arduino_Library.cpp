...
const uint8_t VCNL4040_ADDR = 0x73; //7-bit unshifted I2C address of VCNL4040
...
boolean VCNL4040::begin(uint8_t ir_address, TwoWire &wirePort) {
...
//  if (getID(ir_address) != 0x0186) return (false); //Check default ID value
  ...
  setIRDutyCycle(ir_address, 40); //Set to highest duty cycle

  setProxIntegrationTime(ir_address, 8); //Set to max integration

  setProxResolution(ir_address, 16); //Set to 16-bit output
  
  enableSmartPersistance(ir_address); //Turn on smart presistance

  powerOnProximity(ir_address); //Turn on prox sensing
...
boolean VCNL4040::isConnected(uint8_t ir_address) {
  _i2cPort->beginTransmission((uint8_t)ir_address);
...
void VCNL4040::setIRDutyCycle(uint8_t ir_address, uint16_t dutyValue)
{
  ...
  bitMask(ir_address, VCNL4040_PS_CONF1, LOWER, VCNL4040_PS_DUTY_MASK, dutyValue);
}
...
void VCNL4040::setProxInterruptPersistance(uint8_t ir_address, uint8_t persValue)
{
  bitMask(ir_address, VCNL4040_PS_CONF1, LOWER, VCNL4040_PS_PERS_MASK, persValue);
}
...
void VCNL4040::setAmbientInterruptPersistance(uint8_t ir_address, uint8_t persValue)
{
  bitMask(ir_address, VCNL4040_ALS_CONF, LOWER, VCNL4040_ALS_PERS_MASK, persValue);
}

void VCNL4040::enableAmbientInterrupts(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_ALS_CONF, LOWER, VCNL4040_ALS_INT_EN_MASK, VCNL4040_ALS_INT_ENABLE);
}
void VCNL4040::disableAmbientInterrupts(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_ALS_CONF, LOWER, VCNL4040_ALS_INT_EN_MASK, VCNL4040_ALS_INT_DISABLE);
}

//Power on or off the ambient light sensing portion of the sensor
void VCNL4040::powerOnAmbient(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_ALS_CONF, LOWER, VCNL4040_ALS_SD_MASK, VCNL4040_ALS_SD_POWER_ON);
}
void VCNL4040::powerOffAmbient(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_ALS_CONF, LOWER, VCNL4040_ALS_SD_MASK, VCNL4040_ALS_SD_POWER_OFF);
}

//Sets the integration time for the ambient light sensor
void VCNL4040::setAmbientIntegrationTime(uint8_t ir_address, uint16_t timeValue)
{
  ...
  bitMask(ir_address, VCNL4040_ALS_CONF, LOWER, VCNL4040_ALS_IT_MASK, timeValue);
}

//Sets the integration time for the proximity sensor
void VCNL4040::setProxIntegrationTime(uint8_t ir_address, uint8_t timeValue)
{
  ...
  bitMask(ir_address, VCNL4040_PS_CONF1, LOWER, VCNL4040_PS_IT_MASK, timeValue);
}

//Power on the prox sensing portion of the device
void VCNL4040::powerOnProximity(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_PS_CONF1, LOWER, VCNL4040_PS_SD_MASK, VCNL4040_PS_SD_POWER_ON);
}

//Power off the prox sensing portion of the device
void VCNL4040::powerOffProximity(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_PS_CONF1, LOWER, VCNL4040_PS_SD_MASK, VCNL4040_PS_SD_POWER_OFF);
}

//Sets the proximity resolution
void VCNL4040::setProxResolution(uint8_t ir_address, uint8_t resolutionValue)
{
	if(resolutionValue > 16 - 1) resolutionValue = VCNL4040_PS_HD_16_BIT;
	else resolutionValue = VCNL4040_PS_HD_12_BIT;
	
  bitMask(ir_address, VCNL4040_PS_CONF2, UPPER, VCNL4040_PS_HD_MASK, resolutionValue);
}

//Sets the proximity interrupt type
void VCNL4040::setProxInterruptType(uint8_t ir_address, uint8_t interruptValue)
{
  bitMask(ir_address, VCNL4040_PS_CONF2, UPPER, VCNL4040_PS_INT_MASK, interruptValue);
}
...
void VCNL4040::enableSmartPersistance(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_PS_CONF3, LOWER, VCNL4040_PS_SMART_PERS_MASK, VCNL4040_PS_SMART_PERS_ENABLE);
}
void VCNL4040::disableSmartPersistance(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_PS_CONF3, LOWER, VCNL4040_PS_SMART_PERS_MASK, VCNL4040_PS_SMART_PERS_DISABLE);
}
...
void VCNL4040::enableActiveForceMode(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_PS_CONF3, LOWER, VCNL4040_PS_AF_MASK, VCNL4040_PS_AF_ENABLE);
}
void VCNL4040::disableActiveForceMode(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_PS_CONF3, LOWER, VCNL4040_PS_AF_MASK, VCNL4040_PS_AF_DISABLE);
}

//Set trigger bit so sensor takes a force mode measurement and returns to standby
void VCNL4040::takeSingleProxMeasurement(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_PS_CONF3, LOWER, VCNL4040_PS_TRIG_MASK, VCNL4040_PS_TRIG_TRIGGER);
}

//Enable the white measurement channel
void VCNL4040::enableWhiteChannel(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_PS_MS, UPPER, VCNL4040_WHITE_EN_MASK, VCNL4040_WHITE_ENABLE);
}
void VCNL4040::disableWhiteChannel(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_PS_MS, UPPER, VCNL4040_WHITE_EN_MASK, VCNL4040_WHITE_ENABLE);
}
...
void VCNL4040::enableProxLogicMode(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_PS_MS, UPPER, VCNL4040_PS_MS_MASK, VCNL4040_PS_MS_ENABLE);
}
void VCNL4040::disableProxLogicMode(uint8_t ir_address)
{
  bitMask(ir_address, VCNL4040_PS_MS, UPPER, VCNL4040_PS_MS_MASK, VCNL4040_PS_MS_DISABLE);
}

//Set the IR LED sink current to one of 8 settings
void VCNL4040::setLEDCurrent(uint8_t ir_address, uint8_t currentValue)
{
	...
	bitMask(ir_address, VCNL4040_PS_MS, UPPER, VCNL4040_LED_I_MASK, currentValue);
}

//Set the proximity sensing cancelation value - helps reduce cross talk
//with ambient light
void VCNL4040::setProxCancellation(uint8_t ir_address, uint16_t cancelValue)
{
  writeCommand(ir_address, VCNL4040_PS_CANC, cancelValue);
}

//Value that ALS must go above to trigger an interrupt
void VCNL4040::setALSHighThreshold(uint8_t ir_address, uint16_t threshold)
{
  writeCommand(ir_address, VCNL4040_ALS_THDH, threshold);
}

//Value that ALS must go below to trigger an interrupt
void VCNL4040::setALSLowThreshold(uint8_t ir_address, uint16_t threshold)
{
  writeCommand(ir_address, VCNL4040_ALS_THDL, threshold);
}

//Value that Proximity Sensing must go above to trigger an interrupt
void VCNL4040::setProxHighThreshold(uint8_t ir_address, uint16_t threshold)
{
  writeCommand(ir_address, VCNL4040_PS_THDH, threshold);
}

//Value that Proximity Sensing must go below to trigger an interrupt
void VCNL4040::setProxLowThreshold(uint8_t ir_address, uint16_t threshold)
{
  writeCommand(ir_address, VCNL4040_PS_THDL, threshold);
}

//Read the Proximity value
uint16_t VCNL4040::getProximity(uint8_t ir_address)
{
  return (readCommand(ir_address, VCNL4040_PS_DATA));
}

//Read the Ambient light value
uint16_t VCNL4040::getAmbient(uint8_t ir_address)
{
  return (readCommand(ir_address, VCNL4040_ALS_DATA));
}

//Read the White light value
uint16_t VCNL4040::getWhite(uint8_t ir_address)
{
  return (readCommand(ir_address, VCNL4040_WHITE_DATA));
}

//Read the sensors ID
uint16_t VCNL4040::getID(uint8_t ir_address)
{
  return (readCommand(ir_address, VCNL4040_ID));
}

//Returns true if the prox value rises above the upper threshold
boolean VCNL4040::isClose(uint8_t ir_address)
{
  uint8_t interruptFlags = readCommandUpper(ir_address, VCNL4040_INT_FLAG);
  return (interruptFlags & VCNL4040_INT_FLAG_CLOSE);
}

//Returns true if the prox value drops below the lower threshold
boolean VCNL4040::isAway(uint8_t ir_address)
{
  uint8_t interruptFlags = readCommandUpper(ir_address, VCNL4040_INT_FLAG);
...
//Returns true if the prox value rises above the upper threshold
boolean VCNL4040::isLight(uint8_t ir_address)
{
  uint8_t interruptFlags = readCommandUpper(ir_address, VCNL4040_INT_FLAG);
...
boolean VCNL4040::isDark(uint8_t ir_address)
{
  uint8_t interruptFlags = readCommandUpper(ir_address, VCNL4040_INT_FLAG);
...
uint16_t VCNL4040::readCommand(uint8_t ir_address, uint8_t commandCode)
{
  _i2cPort->beginTransmission(ir_address);
  ...
  _i2cPort->requestFrom(ir_address, (uint8_t)2);
...
boolean VCNL4040::writeCommand(uint8_t ir_address, uint8_t commandCode, uint16_t value)
{
  _i2cPort->beginTransmission(ir_address);
  _i2cPort->write(commandCode);
...
boolean VCNL4040::writeCommandLower(uint8_t ir_address, uint8_t commandCode, uint8_t newValue)
{
  uint16_t commandValue = readCommand(ir_address, commandCode);
  commandValue &= 0xFF00; //Remove lower 8 bits
  commandValue |= (uint16_t)newValue; //Mask in
  return (writeCommand(ir_address, commandCode, commandValue));
}

//Given a command code (address) write to the upper byte without affecting the lower byte
boolean VCNL4040::writeCommandUpper(uint8_t ir_address, uint8_t commandCode, uint8_t newValue)
{
  uint16_t commandValue = readCommand(ir_address, commandCode);
...
uint8_t VCNL4040::readCommandLower(uint8_t ir_address, uint8_t commandCode)
{
  uint16_t commandValue = readCommand(ir_address, commandCode);
...
uint8_t VCNL4040::readCommandUpper(uint8_t ir_address, uint8_t commandCode)
{
  uint16_t commandValue = readCommand(ir_address, commandCode);
...
void VCNL4040::bitMask(uint8_t ir_address, uint8_t commandAddress, boolean commandHeight, uint8_t mask, uint8_t thing)
{
  // Grab current register context
  uint8_t registerContents;
  if (commandHeight == LOWER) registerContents = readCommandLower(ir_address, commandAddress);
  else registerContents = readCommandUpper(ir_address, commandAddress);
  ...
  // Change contents
  if (commandHeight == LOWER) writeCommandLower(ir_address, commandAddress, registerContents);
  else writeCommandUpper(ir_address, commandAddress, registerContents);
}
