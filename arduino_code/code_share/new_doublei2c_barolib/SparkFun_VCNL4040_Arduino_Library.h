...
class VCNL4040 {
  ...
  public:
    ...
    boolean begin(uint8_t ir_address = 0x75, TwoWire &wirePort = Wire);
    boolean isConnected(uint8_t ir_address = 0x75); //True if sensor responded to I2C query

    void setIRDutyCycle(uint8_t ir_address, uint16_t dutyValue);

    void setProxInterruptPersistance(uint8_t ir_address, uint8_t persValue);
    void setAmbientInterruptPersistance(uint8_t ir_address, uint8_t persValue);

    void setProxIntegrationTime(uint8_t ir_address, uint8_t timeValue); //Sets the integration time for the proximity sensor
    void setAmbientIntegrationTime(uint8_t ir_address, uint16_t timeValue);

    void powerOnProximity(uint8_t ir_address); //Power on the prox sensing portion of the device
    void powerOffProximity(uint8_t ir_address); //Power off the prox sensing portion of the device

    void powerOnAmbient(uint8_t ir_address); //Power on the ALS sensing portion of the device
    void powerOffAmbient(uint8_t ir_address); //Power off the ALS sensing portion of the device

    void setProxResolution(uint8_t ir_address, uint8_t resolutionValue);//Sets the proximity resolution to 12 or 16 bit

    void enableAmbientInterrupts(uint8_t ir_address);
    void disableAmbientInterrupts(uint8_t ir_address);

    void enableSmartPersistance(uint8_t ir_address);
    void disableSmartPersistance(uint8_t ir_address);

    void enableActiveForceMode(uint8_t ir_address);
    void disableActiveForceMode(uint8_t ir_address);
    void takeSingleProxMeasurement(uint8_t ir_address);

    void enableWhiteChannel(uint8_t ir_address);
    void disableWhiteChannel(uint8_t ir_address);

    void enableProxLogicMode(uint8_t ir_address);
    void disableProxLogicMode(uint8_t ir_address);

    void setLEDCurrent(uint8_t ir_address, uint8_t currentValue);

    void setProxCancellation(uint8_t ir_address, uint16_t cancelValue);
    void setALSHighThreshold(uint8_t ir_address, uint16_t threshold);
    void setALSLowThreshold(uint8_t ir_address, uint16_t threshold);
    void setProxHighThreshold(uint8_t ir_address, uint16_t threshold);
    void setProxLowThreshold(uint8_t ir_address, uint16_t threshold);

    uint16_t getProximity(uint8_t ir_address);
    uint16_t getAmbient(uint8_t ir_address);
    uint16_t getWhite(uint8_t ir_address);
    uint16_t getID(uint8_t ir_address);

    void setProxInterruptType(uint8_t ir_address, uint8_t interruptValue); //Enable four prox interrupt types
    boolean isClose(uint8_t ir_address); //Interrupt flag: True if prox value greater than high threshold
    boolean isAway(uint8_t ir_address); //Interrupt flag: True if prox value lower than low threshold
    boolean isLight(uint8_t ir_address); //Interrupt flag: True if ALS value higher than high threshold
    boolean isDark(uint8_t ir_address); //Interrupt flag: True if ALS value lower than low threshold

    uint16_t readCommand(uint8_t ir_address, uint8_t commandCode);
    uint8_t readCommandLower(uint8_t ir_address, uint8_t commandCode);
    uint8_t readCommandUpper(uint8_t ir_address, uint8_t commandCode);

    boolean writeCommand(uint8_t ir_address, uint8_t commandCode, uint16_t value);
    boolean writeCommandLower(uint8_t ir_address, uint8_t commandCode, uint8_t newValue);
    boolean writeCommandUpper(uint8_t ir_address, uint8_t commandCode, uint8_t newValue);

    void bitMask(uint8_t ir_address, uint8_t commandAddress, boolean commandHeight, uint8_t mask, uint8_t thing);
